...
void* memset(void *s, int chr, size_t size) {
	char *ss = (char *) s;
	for (size_t i = 0; i < size; ++i) ss[i] = chr;
	return s;
}

// functions to handle sigset_t data types
...
int sigismember(const sigset_t *set, int signo) {
	...
	return sigmask(signo) & set->sig[0];
}

// signal and sigaction
int sigaction(int sig, struct sigaction *act, struct sigaction *oact) {
	act->sa_flags |= SA_RESTORER;
	act->sa_restorer = sys_sigret;
...
// alarm wrapper
unsigned int alarm(unsigned sec) {
